...
function MouseModel({ onLoad }) {
    const { scene } = useGLTF(threeAssetUrls.mouse, true);
    ...
    return (
        <primitive 
            ...
            position={[0, 0, 0]}
            rotation={[0, 0, 0]}
            scale={threeAssetMetadata.mouse.scale}
        />
    );
}
...
function Mouse3D() {
    ...
    return (
        <div className="phone-3d-container">
            <h2 className='titolo-bianco text-9xl   font-extrabold  absolute top-1/2 left-2/3 -translate-x-1/2 -translate-y-1/2 rotate-90 z-[998] inter'>MOUSE</h2>
            <ErrorBoundary fallback={<ErrorFallback />}>
                <Suspense fallback={<LoadingFallback />}>
                    <Canvas
                        ...
                        camera={{ position: threeAssetMetadata.mouse.cameraPosition, fov: 70 }}
                        gl={{
                            antialias: true,
                            alpha: true,
